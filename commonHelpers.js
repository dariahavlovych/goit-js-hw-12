...
</li>`).join(""),new n(".gallery-item a",{className:"js-lightbox",overlayOpacity:.8,captionsData:"alt",captionDelay:250})}function m(s){const o=new URLSearchParams({key:"45077643-3bb964d60cb084522e3de8d92",q:s,image_type:"photo",orientation:"horizontal",safesearch:!0});return fetch(`https://pixabay.com/api/?${o}`).then(t=>{if(!t.ok)throw new Error(t.status);return t.json()})}const f=document.querySelector(".search-form");f.addEventListener("submit",h);function h(s){s.preventDefault();const o=s.target.elements.search.value.trim().toLowerCase();o&&m(o).then(t=>{t.hits.length===0&&c.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight",timeout:2e3,icon:""}),u(t.hits)}).catch(t=>console.log(t)).finally(s.target.reset())}
